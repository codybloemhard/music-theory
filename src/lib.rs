...
pub fn notes_analysis(string: String){
    ...
    println!("----------------------------------------");
    println!("\tSubchords:");
    RootedChord::from_scale(scale).into_sub_chords()
    ...
    for modeobj in find_chordscales(steps.clone()){
        if included.contains(&modeobj.steps) { continue; }
        included.insert(modeobj.steps.clone());
        println!("{} {}: {:?}", ucns[0], modeobj, modeobj.steps.clone().into_scale(root).into_ucns());
        print!("\t");
    ...
    println!("\tSupersets:");
    for (tonic,modeobj) in find_scale_superset(ucns.clone(), true){
        if included.contains(&modeobj.steps) { continue; }
        included.insert(modeobj.steps.clone());
    ...
    for modeobj in find_scale_superseq(&steps){
        if included.contains(&modeobj.steps) { continue; }
        included.insert(modeobj.steps.clone());
        println!("{}", modeobj);
    }
